...
public class DiapersPage extends JFrame {

    private static final long serialVersionUID = 1L;

    public DiapersPage() throws Exception {
        ...
        btnNewButton.addActionListener(new ActionListener() { //  performs a certain action when interacted
            public void actionPerformed(ActionEvent a) {
                ...
                switch(msg) {
                    case "Carousell":
                        try {
                            HistoItems histoSocks = new HistoItems("Diaper", "DiaperCarousell", "DiaperQoo10", "DiapersRakuten"); // generates graph
                            histoSocks.pack();
                            RefineryUtilities.centerFrameOnScreen(histoSocks);
                            histoSocks.setVisible(true);

                            final NormalDist normal = new NormalDist("DiaperCarousell","Diapers", "DiaperCarousell"); // generate graph
                            normal.pack();
                    ...
                    case "Qoo10":
                        try {
                            HistoItems histoSocks = new HistoItems("Diaper", "DiaperCarousell", "DiaperQoo10", "DiapersRakuten");
                            histoSocks.pack();
                            RefineryUtilities.centerFrameOnScreen(histoSocks);
                            histoSocks.setVisible(true);

                            final NormalDist normal = new NormalDist("DiaperQoo10","Diapers", "DiaperQoo10");
                            normal.pack();
                            RefineryUtilities.centerFrameOnScreen(normal);
                            normal.setVisible(true);
                            break;
                    ...
                    case "Rakuten":
                        try {
                            ...
                            final NormalDist normal = new NormalDist("DiapersRakuten","Diapers", "DiapersRakuten");
                            normal.pack();
                            RefineryUtilities.centerFrameOnScreen(normal);
                            normal.setVisible(true);
                            break;
        ...
        JLabel lblNewLabel_2 = new JLabel(new ImageIcon("Price-icon.png")); // produce image for our application
        ...
        DefaultTableModel model = new DefaultTableModel(0, 0); // model for table to reference
        ...
        callCarousell.query("DiaperCarousell"); // call method for query
        ...
        callQoo10.query("DiaperQoo10");// call method for query which creates a array list for qoo10
        ...
        callRakuten.query("DiapersRakuten");// call method for query which creates a array list for rakuten
        ...
        table.addMouseListener(new MouseAdapter() { // allow links to be clickable
            ...
            private void open(URI uri) { // method for opening url
        ...
        JScrollPane scrollPane = new JScrollPane(table); // gives the table scrollable features
        ...
        JScrollPane scrollPane2 = new JScrollPane(table1); // gives the table scrollable features
        scrollPane.setVerticalScrollBarPolicy ( ScrollPaneConstants.VERTICAL_SCROLLBAR_ALWAYS );
