...
class RakutenDiapers extends WebHelper implements Runnable{
    ...
    public RakutenDiapers(String databaseName, String keyword1, String keyword2, String keyword3){
    ...
    public void crawl(){
        try
        {
            ...
            for(int i=1;i<=6;i++) {
                if(i == 1) {
                    url = "https://global.rakuten.com/en/search/?tl=0&k=diapers&l-id=search_regular"; // other than first time... url will be url = page.getNextUrl(url)
                    int counter = 0;
                    ...
                    for (Element rows : row) // loop for each element
                    {
    ...
    @Override
    public void run() { // make a new thread
        crawl();
    }
}